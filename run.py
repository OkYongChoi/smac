...
def standard_run(_config, _log, game_name):
    ...
    unique_token = f"{args.name}__{datetime.datetime.now():%Y-%m-%d_%H-%M-%S}" 
    args.unique_token = unique_token
    if args.use_tensorboard:
        tb_logs_direc = os.path.join(dirname(dirname(abspath(__file__))), "results", f"tb_logs/{game_name}")
...
def evaluate_sequential(args, runner):
    """
        This function is run when Evaluate: True or save_replay: True.
        evaluate: True and save_replay: False ->  Is only evaluated. The replay is not saved.
        evaluate: False and save_replay: True ->  Is evaluated, and then the replay is not saved.
        evaluate: True and save_replay: True ->  Is evaluated, and then the replay is not saved.
        So, if save_replay: True, you don't have to make evaluate: True
    """
...
def run_sequential(args, logger):
    ...
    # Setup multiagent controller here (basic_controller, separate_controller etc.)
    mac = mac_REGISTRY[args.mac](buffer.scheme, groups, args)

    # Give runner the scheme set above
    runner.setup(scheme=scheme, groups=groups, preprocess=preprocess, mac=mac)

    # Learner (q-learner, coma_learner, etc.)
    ...
    ###### If checkpoint_path is given specifically, and if args.evaluate == True or args.save_replay == True, 
